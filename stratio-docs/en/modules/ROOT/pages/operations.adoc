...
For the specified API management of the _cluster_, the following object groups are created:
...
You can see the new number of replicas and the new Machine objects:
...
Since the autoscaling is based on the _cluster-autoscaler_, the minimum and maximum will be added in the workers node group as annotations:
...
NOTE: The controller provisions a new node in the workers group with the updated version and, once it is ready in Kubernetes, deletes a node with the old version. This way, it always ensures the configured number of nodes.
