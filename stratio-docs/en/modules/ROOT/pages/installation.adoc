...
** xref:attachment$stratio-eks-eks-temp-policy.json[Download temporary permissions for EKS]
** xref:attachment$stratio-eks-eks-temp-policy.json[Download temporary permissions for EKS]
...
For deployments on Google Cloud Platform, you will require permissions on _compute_ (instances, disks, images, routers, networks, etc.). Similarly to other supported providers, provisioning requires an account with all requested permissions.
+
...
As it will be shown below, the provider aupports the use of both a VPC and subnets previously created using the networks parameter in the cluster descriptor.
...
Both the VPC and the subnets must be created in the cloud provider. The subnets can be private or public, but if they are public, they must have a _NAT gateway_ and an _Internet Gateway_ in the same VPC. If both types of _subnets_ are specified, the worker nodes will be deployed in private subnets.
...
|flavour
|Installation flavor, which indicates cluster size and resiliency. Default is `production`.
|development
|Yes
